# Cheat Sheet for Analysts — внутренняя база знаний банка

Внутренний веб-сервис для аналитиков банка: единая база знаний, объединяющая методологию анализа (BA / SA / Product Owner, SQL, статистика, Power BI) и **банковскую специфику** — регуляторику ЦБ и комплаенс (AML/KYC, 115-ФЗ), банковские продукты и процессы (счета, карты, платежи, СБП, эквайринг) и интеграции банковских систем (АБС, процессинг, ISO 20022, API).

**Зачем банку:** ускоряет онбординг новых аналитиков, служит единой точкой правды по требованиям и терминам, снижает зависимость от устных знаний. Подходит любому отделу, где работают с требованиями и данными.

**Стек:** Express + SQLite (встроенный node:sqlite) на бэкенде, ванильный JS на фронтенде. Полный CRUD, поиск, тесты (Jest), CI (GitHub Actions).

**Быстрый старт:**
```bash
cd backend && npm install && npm start   # затем открыть http://localhost:3000
```
